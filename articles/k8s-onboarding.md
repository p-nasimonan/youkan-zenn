...
:::message alert
**Docker Desktopのバージョンが古い場合、サイドバーにKubernetesメニューが表示されないことがあります。** その場合は最新バージョンにアップデートしてから再試行してください。
:::
...
:::message
**クラスターとノードの関係**
**クラスター**とは、k8sが管理するサーバー群全体のことです。その中の個々のサーバー（物理機やVM）を**ノード**と呼びます。
kindの場合は、Dockerコンテナ1つが丸ごと1ノードになります。本番環境では複数のノードが束になってクラスターを構成しますが、学習用途では今回のような1ノード構成で十分です。
:::
...
:::message
**`ImagePullBackOff` が表示されたら？**
`kubectl get pods` で `STATUS` が `ImagePullBackOff` や `ErrImagePull` になることがあります。

```text
NAME                                READY   STATUS             RESTARTS   AGE
nginx-deployment-6f9664446b-5222p   0/1     ImagePullBackOff   0          1m
```

これはDocker Hubからイメージをダウンロード中にネットワークエラーが起きたサインです。`kubectl describe pod [Pod名]` を実行すると、Eventsに `short read: expected N bytes but got 0` のようなメッセージが確認できます。

**対処法:** しばらく待つとk8sが自動でリトライし、ほとんどの場合は自然に `Running` へ移行します。数分待っても解決しない場合は **Docker Desktopを再起動**してみてください。kindはコンテナの中でk8sを動かす仕組み上、Docker Desktop側のネットワークスタックの不調がイメージ取得の失敗として現れることがあります。
:::
...
「k8sは技術力の高い人が使うもの」というイメージを持っていた方は多いでしょう。実際に触れてみると、**難しさの正体は「概念の多さ」であって、動かすこと自体はそれほど難しくない**と感じた方もいるのではないでしょうか。

Pod・Deployment・Serviceという3つの概念さえ押さえれば、「コンテナが死んでも自動で復活する」「IPが変わっても通信先が追いかけてくれる」といった便利さが体感できます。最初から全部理解しようとせず、まず動かしてから「なぜこうなるのか」を調べる順番が、k8s習得の近道です。
...
|---|---|
| 障害が起きると手動でコンテナを再起動していた | **自己修復機能**によって勝手に新しいPodが起動した |
| コンテナの増減に合わせてルーターの設定も変えていた | **Serviceとラベルの紐付け**によって、通信先が動的に振り分けられた |
| 本番とローカルで作業手順や環境のズレ（属人化）が生じた | **すべての構成をYAML（状態の宣言）で記述**するため、どこでも同じ状態が再現できた |
