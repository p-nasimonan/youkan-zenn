...
:::message
ymlはインデントで階層を表現するが、ドキュメント等では②、③のように階層をドット記法で表現することが多い
:::
...
外からのアクセスは一旦この「Service」が受け取り、さきほど設定した`spec.template.metadata.labels`の`app: nginx`を目印にして、裏側にいるPodへ通信を自動で振り分けてくれます。
...
3. k8sクラスタ自体の削除  
Docker Desktopのメニューから`k8s`のアイコンをクリックして`stop`を選択することで削除できます。
