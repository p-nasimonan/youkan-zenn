---
title: "【k8s入門】最速で体験する！Kubernetesオンボーディング"
emoji: "☸️"
...
- **ゴール:** DeploymentとServiceの役割を腹落ちして理解し、k8sの「宣言的」なアプローチを体感すること
...
:::message
**宣言的 vs 命令的**
画面には `kubectl create deployment` などのコマンド例が記載されています。このように「〇〇を作れ（どうやるか）」と直接指示を出すのが**命令的**な操作です。
しかし実運用では、後述するYAMLファイルを用いて「この状態になっていてほしい」と定義する**宣言的**な操作がk8sの基本となります。本記事でも宣言的な方法だけを使います。
:::
